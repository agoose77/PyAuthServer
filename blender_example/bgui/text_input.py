...
from .widget import Widget, WeakMethod, BGUI_DEFAULT, BGUI_CENTERY, \
	...
	BGUI_INPUT_MIXED, BGUI_INPUT_TEXT, BGUI_INPUT_NUMERICAL, BGUI_INPUT_FLOAT, \
	BGUI_INPUT_INTEGER
...
class TextInput(Widget):
	...
	theme_options = {
				...
				'BorderSize': 0,
				'BorderColor': (0, 0, 0, 0),
				'ErrorBorderColor': (0.8, 0.0, 0.0, 1.0),
				'ErrorFrameColor': (0.8, 0.4, 0.4, 1.0),
	...
	def __init__(self, parent, name, text="", prefix="", font=None, pt_size=None, color=None,
					...
					type=BGUI_INPUT_MIXED, allow_empty=True):
		"""
	...
	def on_input(self, value):
		self._on_input = WeakMethod(value)
	...
	def invalid(self):
		...
		if callable(self.on_validate):
			...
			if self.type == BGUI_INPUT_TEXT:
				error = not temp.isalpha()
			elif self.type == BGUI_INPUT_FLOAT:
				error = not isfloat
			elif self.type == BGUI_INPUT_INTEGER:
				error = not temp.isdigit()
			elif self.type == BGUI_INPUT_NUMERICAL:
				error = not (temp.isdigit() or isfloat)
			else:
				error = False
			error &= bool(temp)
			error |= not (temp or self.allow_empty)
	...
	def _draw(self):
		...
		if self.invalid:
			...
			self.swapcolors(1 if self.colormode == 2 else self.colormode)
