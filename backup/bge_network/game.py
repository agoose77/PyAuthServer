...
class PhysicsSystem(System):
    ...
    def post_update(self, delta_time):
        ...
        for replicable in WorldInfo.subclass_of(Actor): 
            ...
            # Make note of replicables to restore state
            if replicable.roles.local == Roles.autonomous_proxy:
                to_restore.append(replicable)
                replicable.render_state.save()
        
        for replicable in to_restore:
...
class Loop(types.KX_PythonLogicLoop):
    ...
    def __init__(self):
        ...
        self.network_scene = self.find_scene("Scene")
        self.network = None
        
        self.physics = PhysicsSystem(self)
    
    def find_scene(self, name):
        return next(s for s in logic.getSceneList() if s.name == name)
        ...
        while not self.check_quit():
            ...
            for scene in logic.getSceneList():
                ...
                if not self.pause:
                    ...
                    if scene is self.network_scene and self.network:
                        self.network.update(deltatime)           
