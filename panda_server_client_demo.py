from network.type_serialisers import get_serialiser, TypeInfo

def nested_list(type_info=TypeInfo(int)):
    return TypeInfo(list, item_info=type_info)

flag = nested_list(nested_list())
# Used for packing [[1,2,3], [4,5,6]]
serialiser = get_serialiser(flag)

data = serialiser.pack([[1,2,3], [4,5,6]])
print(len(data))
