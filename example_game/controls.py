...
class HandleThirdPersonCamera(LeafNode):
    ...
    def evaluate(self, blackboard):
        ...
        camera.local_position.y = min(maximum_y, max(minimum_y, camera.local_position.y))
        camera.local_position.z = min(maximum_z, max(minimum_z, camera.local_position.z))

        camera.local_position.length = camera.gimbal_offset

        rotation = Vector((0, -1, 0)).rotation_difference(camera.local_position).inverted().to_euler()
        rotation[0] *= -1
